...
def parseArgs():
    parser = argparse.ArgumentParser(description="Identify natural languages in code")
...
class FunctionDatasetModifyingDocuments:
    ...
    def __call__(self, example):
        fasttext_pred = get_all_fasttext_info(example["nl_text"], self.model_lang_id)
...
def main():
    ...
    dataset = dataset.map(extract_nl_text, num_proc=1) #num_proc=multiprocessing.cpu_count()
